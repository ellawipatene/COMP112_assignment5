...
public class WeatherReporter{
    ...
    public void plotSnapshots(String dataType, List<String> dates,List<Integer> stationAmounts ,List<String> stationNames, List<Double> xCords, List<Double> yCords, List<Double> temp ){
        ...
        int lineCounter = 0; // will be added onto after each snapshot - used to go through x,y coords and making sublists
        int animationCounter = 0; 
        
        ArrayList<Double> newTempList = getAnimationList(temp); 
        
        for (int i = 0; i < snapshots; i++){
            int stations = stationAmounts.get(i); 
            
            // List<Double> tempSnapshot = newTempList.subList(lineCounter, lineCounter + (stations*4)); // makes a list with just the temps of that single snapshot
            ...
            if (i == snapshots - 1){   // If it is the last snapshot, because the last one only has one value (not 4 for the animation) 
                ...
                for (int x = 0; x < stations; x++){ // will draw a circle for each station
                    List<Double> tempSnapshot = newTempList.subList(animationCounter, animationCounter + stations);
                    double maxTemp = getMaxTemp(tempSnapshot);
                    double minTemp = getMinTemp(tempSnapshot);
                    
                    // has a different colour range for each data type. 
                    if (dataType.equals("temp")){ 
                        UI.setColor(getTemperatureColor(newTempList.get(animationCounter))); 
                    } else if (dataType.equals("seaKPa")){
                        UI.setColor(getSeaKPaColor(newTempList.get(animationCounter))); 
                    } else if (dataType.equals("kPa")){
                        UI.setColor(getKPaColor(newTempList.get(animationCounter)));
                    } else if (dataType.equals("dew")){
                        UI.setColor(getDewColor(newTempList.get(animationCounter)));
                    }
                    // draws the circles 
                    if (newTempList.get(animationCounter) == -999){
                        UI.drawOval(xCords.get(lineCounter), yCords.get(lineCounter), DIAM, DIAM); 
                    }else if (newTempList.get(animationCounter) == maxTemp || newTempList.get(animationCounter) == minTemp){
                ...
                for (int y = 0; y < 6; y++){
                    ...
                    List<Double> tempSnapshot = newTempList.subList(animationCounter, animationCounter + stations); // makes a list with just the temps of that single snapshot
                    // I made a sublist so I could calculate the min and max for each snapshot
                    double maxTemp = getMaxTemp(tempSnapshot);
                    double minTemp = getMinTemp(tempSnapshot); // most of the mins do not show because the location is off the map
                    
                    for (int x = 0; x < stations; x++){
                        // has a different colour range for datatype. 
                        if (dataType.equals("temp")){
                            UI.setColor(getTemperatureColor(newTempList.get(animationCounter))); 
                        } else if (dataType.equals("seaKPa")){
                            UI.setColor(getSeaKPaColor(newTempList.get(animationCounter))); 
                        ...
                        lineCounter++; // line counter for x and y values in the list
                        animationCounter++; // animation counter for the data values in the list
                    }
                    if (y < 5){ // resets the line counter so that it has the same x and y values 
                        lineCounter = lineCounter - stations;
                        UI.println(lineCounter); 
                    }
                    UI.sleep(50); // pause 
                } 
                UI.sleep(200);  // pause slightly longer for the actual values
            }
        } 
    }
    ...
    public ArrayList<Double> getAnimationList(List<Double> orgList){
        ...
        for (int i = 0; i < listLength-1; i++){
            ...
            Double diff = two - one; 
            Double aveDiff = diff/6; // works out the average difference between the two values
            ...
            newList.add(one + (aveDiff * 4));
            newList.add(one + (aveDiff * 5));
        }
